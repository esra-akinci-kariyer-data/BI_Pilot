...
TREND_REPORTS = [
    ...
    {"name": "Canlı İKÇO Dashboard", "path": "Operasyon/Cagri"},
    {"name": "Aylık Churn", "path": "Musteri/Analiz"}
]
...
def get_ai_prediction(name, path, model_name):
    try:
        ...
        prompt = f"Rapor: '{name}' (Yol: {path}). İçeriği ve KPI'ları tahmin et. Format: AÇIKLAMA: ... KPI: ..."
        res = model.generate_content(prompt).text
        d = re.search(r"AÇIKLAMA:(.*)", res, re.I)
...
def search_reports(df_p, df_m, query):
    ...
    for _, r in df_p.iterrows():
        ...
        if any(t in txt for t in qn.split()):
            found.append({"name": r.get("Name"), "topic": r.get("Ana Konu"), "why": r.get("Bu rapor neyi gösteriyor?"), "kpis": r.get("Ana KPI'lar"), "path": r.get("Path"), "verified": True})
...
def render_trend_card(report, key):
    ...
    <div class="recommend-card">
        <div class="trend-label">Gözde</div>
...
def render_drilldown(tree, current_level=0):
    ...
    for i, (key, sub) in enumerate(tree.items()):
        ...
        if st.button(f"📁 {key}", key=f"drill_{current_level}_{i}"):
...
# --- TOP RECOMMENDATIONS ---
st.markdown('<div class="section-title">🔥 Sizin İçin Önerilenler</div>', unsafe_allow_html=True)
t_cols = st.columns(5)
for i, rep in enumerate(TREND_REPORTS):
    with t_cols[i]: render_trend_card(rep, i)

# --- DRILLDOWN ---
st.markdown('<div class="section-title">📂 Rapor Kütüphanesinde Gezin</div>', unsafe_allow_html=True)
render_drilldown(tree)

# --- SEARCH ---
st.markdown('<div class="section-title">🔍 Detaylı Rapor Sorgula</div>', unsafe_allow_html=True)
q_in = st.text_input("İhtiyacınızı yazın (Örn: miktar, churn, hedef...)", value=st.session_state.query)
...
if st.session_state.query:
    ...
    if matches:
        st.markdown('<div class="section-title">Bulunan Sonuçlar</div>', unsafe_allow_html=True)
        for m in matches:
            ...
            <div class="result-card">
                ...
                <div class="field-label">İçerik Analizi</div><div class="field-value">{why}</div>
                <div class="field-label">Temel Metrikler</div><div class="field-value">{kpis}</div>
                <div class="field-label">Klasör Yolu</div><div class="field-value" style="font-size:0.75rem; color:#9ca3af;">{m['path']}</div>
            </div>
            """, unsafe_allow_html=True)