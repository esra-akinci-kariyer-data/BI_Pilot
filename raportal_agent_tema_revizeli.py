...
def render_isin_olsun_otomasyonu():
    st.markdown('<div class="section-title">🚀 İşin Olsun Aylık Otomasyon</div>', unsafe_allow_html=True)
    st.caption("DWH üzerinden aylık rakamları çeker ve Excel raporunu otomatik doldurur.")
    ...
    with col1:
        ...
        if st.button("📤 Seçili Kişiye Mail At", use_container_width=True):
            if not excel_path or not os.path.exists(excel_path):
                ...
                st.info(f"Email gönderme özelliği yapılandırılması gerekmektedir. Hedef: {target_email}")
    
    with col2:
        st.markdown('<div style="background-color: #f0f4f8; padding: 20px; border-radius: 15px; border-left: 5px solid #8c28e8;">', unsafe_allow_html=True)
